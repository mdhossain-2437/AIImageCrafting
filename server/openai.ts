...
if (!apiKey) {
	throw new Error(
		"OPENAI_API_KEY is not set in environment variables. " +
			"Please ensure your .env file exists and contains a valid API key."
	);
}
		...
		if (error.response && error.response.data) {
			...
			if (openaiError) {
				if (openaiError.code === "content_policy_violation") {
					throw new Error(
						"Your prompt violates content policy. Please avoid requests for harmful, illegal, or unethical content."
					);
